...
if (have_posts()):?>
<article class="c-content-wrap">
    <div class="c-insert-img">
        <?php echo get_thumb();?>
    </div>
    <h1 class="c-large-heading c-large-heading:single">
        <?php the_title();?>
    </h1>
    <?php while (have_posts()): the_post();?>
    <div class="c-entry">
        <?php the_content();?>
        <h3 class="c-font-memo">【Content Data】</h3>
        <dl class="c-table">
            <dt class="c-table__title">
                題名
            </dt>
            <dd class="c-table__content">
                <?php echo get_post_meta($post->ID, 'myth_title', true);?>
            </dd>
            <dt class="c-table__title">
                出典
            </dt>
            <dd class="c-table__content">
                <?php echo get_post_meta($post->ID, 'myth_src', true);?>
            </dd>
            <dt class="c-table__title">
                制作者
            </dt>
            <dd class="c-table__content">
                <?php echo get_post_meta($post->ID, 'myth_author', true);?>
            </dd>
            <dt class="c-table__title">
                国・地域
            </dt>
            <dd class="c-table__content">
                <?php echo get_post_meta($post->ID, 'myth_area', true);?>
            </dd>
            <dt class="c-table__title">
                成立時期
            </dt>
            <dd class="c-table__content">
                <?php echo get_post_meta($post->ID, 'myth_era', true);?>
            </dd>
            <dt class="c-table__title">
                関連タイトル
            </dt>
            <dd class="c-table__content">
                <?php echo get_post_meta($post->ID, 'content_r_title', true);?>
            </dd>
            <dt class="c-table__title">
                作業状況
            </dt>
            <dd class="c-table__content">
                <?php $status = get_post_meta($post->ID, 'memo_status', true);
        if ($status) {
            echo $status;
        } else {
            echo '作業中';
        ...
        <div class="o-stack">
            <span>初稿:<time class="c-time"
                    datetime="<?php the_time('Y-m-d')?>"><?php the_time('Y年m月d日')?></time></span>
            <span>改稿:<time class="c-time"
    ...
    <footer class="o-switcher o-switcher:author">
